...
def mergeFiles():
	...
	for p in common.games[game]:
	        for logType in common.logTypes:
				cmd = ""
	            logTypeDir = "/user/flume/Games/"+game+"/"+p+"/"+logType
	            impalaGameDir = "/user/impala/Games/"
	            impalaDir = impalaGameDir+game+"/"+p+"/"+logType
	            fileName = yestoday+".log"
            
	            localDir = "/opt/Games/"+game+"/"+p+"/"+logType
				...
				for dir1 in p3.stdout:
	                ...
	                if code!=0:
	                    ...
	                    continue
	                cmd = "hdfs dfs -chown -R impala "+impalaGameDir
					code = subprocess.call(cmd,shell=True)
	                if code!=0:
	                ...
	                cmd = "impala-shell -i slave1 --query=\"alter table "+tmpTable+" add partition (pdate='"+yestoday+"',ppartner='"+p+"')\""
	                code = subprocess.call(cmd,shell=True)
	                if code !=0:
	                    logErr(game,logType,yestoday,6,cmd)
	                    continue
	                cmd = "impala-shell -i slave1 --query=\"load data inpath '"+impalaDir+"/"+fileName+"' into table "+tmpTable+" PARTITION (pdate='"+yestoday+"',ppartner='"+p+"') \""
	                code = subprocess.call(cmd,shell=True)
	                if code !=0:
	                    logErr(game,logType,yestoday,7,cmd)
	                    continue
	                cmd = "impala-shell -i slave1 --query=\"INSERT INTO "+table+" PARTITION(pdate,ppartner) SELECT * FROM "+tmpTable+"\""
	                code = subprocess.call(cmd,shell=True)
	                if code !=0:
